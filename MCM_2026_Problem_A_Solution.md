...
The discharge curves (generated from `run_mcm_analysis.py`) demonstrate the **non-linear behavior** of Li-ion battery discharge:

### Why Curves Appear Nearly Linear (But Are Not)

The left panel shows full-range discharge curves that appear nearly linear. This is **physically correct** for Li-ion batteries because:

1. **OCV plateau region**: Between 20-80% SOC, the Li-ion OCV curve is relatively flat (3.6V-4.0V)
2. **Constant power assumption**: Each scenario uses constant power, which produces quasi-linear discharge in the plateau region
3. **Non-linearity at extremes**: The curves show subtle curvature, most visible at low SOC

### Low SOC Non-Linearity (Right Panel)

The zoomed right panel focuses on the **low SOC region (<30%)** where non-linearity becomes apparent:

1. **Accelerated discharge**: Below 20% SOC, the voltage drops more rapidly
2. **Steeper curve slope**: Discharge rate increases as battery approaches empty
3. **Physics**: Lower voltage means higher current for same power, causing faster SOC depletion

### OCV-SOC Analysis

![OCV Analysis](pictures/mcm_ocv_analysis.png)

The OCV analysis confirms the physical behavior:

1. **Left plot**: OCV drops steeply below 20% SOC (from ~3.6V to ~3.1V)
2. **Right plot**: Discharge rate is **1.2× faster** at 10% SOC compared to 80% SOC
3. **Model validity**: This matches real Li-ion cell behavior and literature

**Key insight**: Discharge curves are approximately linear in the middle range but show measurable non-linearity at low SOC, consistent with the governing equation:

$$\frac{dSOC}{dt} = -\frac{P_{total}}{V(SOC) \cdot Q_{eff}}$$

When $V(SOC)$ drops at low SOC, $|dSOC/dt|$ increases, causing accelerated discharge.
